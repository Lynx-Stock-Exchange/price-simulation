    ...
    public void consumeAdminCommand(Map<String, Object> commandPayload) {
        try {
            ...
            switch (action.toUpperCase()) {
                ...
                case "SET_SPEED" -> {
                    Object multiplierRaw = commandPayload.get("multiplier");
                    if (multiplierRaw == null) {
                        log.warn("SET_SPEED command missing required field: multiplier");
                        return;
                    }
                    try {
                        int multiplier = Integer.parseInt(multiplierRaw.toString());
                        tickScheduler.setSpeedMultiplier(multiplier);
                    } catch (NumberFormatException e) {
                        log.warn("SET_SPEED command has invalid multiplier value: {}", multiplierRaw);
                    }
                }
